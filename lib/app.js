...
const prompt = () => inquirer.prompt({
  ...
  choices: [
    ...
    constants.CHOICE_COMPARE_DB_DISK,
    constants.CHOICE_MARK_MISSING_FOR_REDOWNLOAD,
    constants.CHOICE_COMPARE_VIMEO_DB_DISK,
  ...
  .then(answer => {
    ...
    switch (answer.action) {
      case constants.CHOICE_DOWNLOAD: require('./download'); break;
      ...
      case constants.CHOICE_RESET_DOWNLOADING:
        return databaseUtil.resetDownloadingVideos()
          .then(n => logger.info(`${n} video(s) reset`))
          .then(() => prompt());
      case constants.CHOICE_PRINT_LIST:
        return printUtil.all().then(() => prompt());
      case constants.CHOICE_COMPARE_DB_DISK: {
        const downloadDir = process.env.DOWNLOAD_DEST || 'E:/vimeo-downloads';
        return printUtil.compareDbAndDisk(downloadDir).then(() => prompt());
      }
      case constants.CHOICE_MARK_MISSING_FOR_REDOWNLOAD: {
        const downloadDir = process.env.DOWNLOAD_DEST || 'E:/vimeo-downloads';
        return printUtil.markMissingFilesForRedownload(downloadDir, databaseUtil).then(() => prompt());
      }
      case constants.CHOICE_COMPARE_VIMEO_DB_DISK: {
        const downloadDir = process.env.DOWNLOAD_DEST || 'E:/vimeo-downloads';
        return printUtil.compareVimeoDbDisk(downloadDir, vimeoUtil, databaseUtil).then(() => prompt());
      }
      case constants.CHOICE_REFRESH:
        return vimeoUtil.fetchAndInsert().then(() => prompt());
      default:
    }
  })
