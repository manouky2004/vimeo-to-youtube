...
const fetchAll = ({ perPage, fields }) => () => findLastPage(perPage)
  ...
  .then(datas => {
    ...
    const allVideos = [].concat(...datas)
    console.log(`You have ${allVideos.length} video(s) on your Vimeo account.`)

    // Compute the path field for each video
    return allVideos.map(video => {
      if (video.name) {
        video.path = `${video.name.replace(/[^a-zA-Z0-9]/g, '_')}.mp4`
      } else {
        console.warn(`Video with resource_key ${video.resource_key} is missing a name.`)
      }
      return video
    })
  })
